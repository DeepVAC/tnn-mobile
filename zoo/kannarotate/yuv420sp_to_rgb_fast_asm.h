...
static void yuv420sp_to_bgr_fast_asm(const unsigned char* yuv420sp, int w, int h, unsigned char* rgb)
{
    const unsigned char* yptr = yuv420sp;
    const unsigned char* vuptr = yuv420sp + w * h;
    ...
    for (int y=0; y<h; y+=2)
    {
        ...
        for (; nn>0; nn--)
        {
            ...
            uint8x8x3_t _rgb0;
            //_rgb0.val[0] = vqshrun_n_s16(_r0, 6); // src
            //_rgb0.val[1] = vqshrun_n_s16(_g0, 6); // src
            //_rgb0.val[2] = vqshrun_n_s16(_b0, 6); // src
            _rgb0.val[0] = vqshrun_n_s16(_b0, 6);
            _rgb0.val[1] = vqshrun_n_s16(_g0, 6);
            _rgb0.val[2] = vqshrun_n_s16(_r0, 6);

            uint8x8x3_t _rgb1;
            //_rgb1.val[0] = vqshrun_n_s16(_r1, 6); // src
            //_rgb1.val[1] = vqshrun_n_s16(_g1, 6); // src
            //_rgb1.val[2] = vqshrun_n_s16(_b1, 6); // src
...
#define SATURATE_CAST_UCHAR(X) (unsigned char)std::min(std::max(X, 0), 255);
            ...
            //int y00 = yptr0[0] << 6;  // src
            //rgb0[0] = SATURATE_CAST_UCHAR((y00 + ruv) >> 6); // src
            //rgb0[1] = SATURATE_CAST_UCHAR((y00 + guv) >> 6); // src
            //rgb0[2] = SATURATE_CAST_UCHAR((y00 + buv) >> 6); // src
            //
            //int y01 = yptr0[1] << 6; // src
            //rgb0[3] = SATURATE_CAST_UCHAR((y01 + ruv) >> 6); // src
            //rgb0[4] = SATURATE_CAST_UCHAR((y01 + guv) >> 6); // src
            //rgb0[5] = SATURATE_CAST_UCHAR((y01 + buv) >> 6); // src
            //
            //int y10 = yptr1[0] << 6; // src
            //rgb1[0] = SATURATE_CAST_UCHAR((y10 + ruv) >> 6); // src
            //rgb1[1] = SATURATE_CAST_UCHAR((y10 + guv) >> 6); // src
            //rgb1[2] = SATURATE_CAST_UCHAR((y10 + buv) >> 6); // src
            //
            //int y11 = yptr1[1] << 6; // src
            //rgb1[3] = SATURATE_CAST_UCHAR((y11 + ruv) >> 6); // src
            //rgb1[4] = SATURATE_CAST_UCHAR((y11 + guv) >> 6); // src
            //rgb1[5] = SATURATE_CAST_UCHAR((y11 + buv) >> 6); // src
...
static void bgr_to_rgb_fast_asm(uint8_t* bgr, int w, int h, uint8_t* rgb){

#if __ARM_NEON
#if __aarch64__
    const int total_bytes = h * w * 3;
    const int stride_bytes = 48;

    for (int i = 0; i < total_bytes; i += stride_bytes)
    {
        uint8_t *bgrPtr = bgr + i;
        uint8_t *targetPtr = rgb + i;

        // swap R and B channel with NEON
        uint8x16x3_t a = vld3q_u8(bgrPtr);
        uint8x16x3_t b;
        b.val[0] = a.val[2];
        b.val[1] = a.val[1];
        b.val[2] = a.val[0];
        vst3q_u8(targetPtr, b);
    }
#else
//    int total_bytes = h * w;
//    __asm__ volatile(
...
//    "bne         0b            \n"
//    :
//    : "r"(bgr), "r"(total_bytes),"r"(rgb)
//    : "r4", "r5"
//    );
    int numOfPixels = w * h;
    for(int i = 0; i < numOfPixels; i+=3)
    {
        rgb[i] = bgr[i] + bgr[i + 2]; // 不能通过复制进行交换，可能bgr和rgb是同一个指针 ？？？
        rgb[i+2] = rgb[i] - bgr[i+2];
        rgb[i] = rgb[i] - rgb[i+2];
    }
#endif // __aarch64__
#endif // __ARM_NEON
...
//    }
//}
//void neon_asm_convert_BGR_TO_RGB(u_char * img, int numPixels)
//{
